...
__global__ void cosine60_60_1_1(double *B_d, double *radius_d)
{
	int blockId = (gridDim.x * blockIdx.y) + blockIdx.x;
	int thread_index = (blockId * blockDim.x) + threadIdx.x;
...
int main()
{
	...
	outputfile = fopen("./outputs/cosine60_60_1_1.txt", "w"); 
	if (outputfile == NULL) {
	...
	cosine60_60_1_1<<< blocks, threads >>>(B_d, radius_d);
